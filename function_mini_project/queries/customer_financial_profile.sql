...
CREATE FUNCTION get_customer_financial_profile
(
    p_customer_id INT
)
RETURNS VARCHAR(500)
...
BEGIN
    ...
    (
        SELECT
            ...
            COUNT(cd.card_id) AS total_card_count,

            COUNT(
                CASE
                    WHEN cd.status = 'Active' THEN cd.card_id
                END
    ...
    ELSEIF v_total_balance >= (v_total_active_loan_amount * 2) THEN
    ...
    SET v_result_message = CONCAT(
        ...
        v_active_loan_count,
        ' | Loan Amount: ',
        ROUND(v_total_active_loan_amount,2),
        ' | Cards: ',
        v_total_card_count,
        ' | Active Cards: ',
        v_active_card_count,
